...
export function mountScrollWorld(container, config) {
  // SPA support: track window listeners + rAF so unmounting a route can tear
  // the world down cleanly (see the returned destroy()).
  let dead = false;
  const winListeners = [];
  const on = (ev, fn, opts) => { window.addEventListener(ev, fn, opts); winListeners.push([ev, fn, opts]); };
  ...
  function raf() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      s.cur += (s.target - s.cur) * (reduce ? 1 : (config.smoothing || 0.18));
